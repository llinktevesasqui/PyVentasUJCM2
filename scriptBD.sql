...
CREATE TABLE IF NOT EXISTS `bdVentas2023`.`productos` (
  ...
  `id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_productos_cartas1_idx` (`id` ASC) ,
  CONSTRAINT `fk_productos_cartas1`
    FOREIGN KEY (`id`)
    REFERENCES `bdVentas2023`.`marcas` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `bdVentas2023`.`detalles_boletas` (
  ...
  INDEX `fk_detalles_boletas_productos1_idx` (`id` ASC) ,
  ...
  CONSTRAINT `fk_detalles_boletas_productos1`
    FOREIGN KEY (`id`)
    REFERENCES `bdVentas2023`.`productos` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
START TRANSACTION;
USE `bdVentas2023`;
INSERT INTO `bdVentas2023`.`empleados` (`idempleados`, `nombres`, `apellidos`, `fecha_nacimiento`, `dni`, `fecha_alta`, `tipo`, `login`, `password`) VALUES (1, 'Walter', 'Coayla', '30/05/1973', '0124578', '01/05/2023', NULL, NULL, NULL);
